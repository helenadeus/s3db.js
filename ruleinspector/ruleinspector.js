...
function ruleInpector(rules) {
	...
	for (var i=0; i<rules.length; i++) {
		...
		var coldata = {'rule_id':rules[i].rule_id, 
			'subject':
			'<span id="subject_'+rules[i].rule_id+'" value="'+rules[i].subject_id+'">'+rules[i].subject+' <FONT SIZE="1px">(C'+rules[i].subject_id+')</FONT></span>', 
			'predicate':'<span id="verb_'+rules[i].rule_id+'" value="'+rules[i].verb_id+'">'+rules[i].verb+'<font size="1px">(I'+rules[i].verb_id+')</FONT></span> ', 
			'object':'<span id="object_'+rules[i].rule_id+'" value="'+((rules[i].object_id!=='')?rules[i].object_id:rules[i].object)+'">'+rules[i].object+((rules[i].object_id!='')?'<FONT SIZE="1px">(C'+rules[i].object_id+')</FONT></span> ':''), 'created_on':rules[i].created_on.substring(0, 10), 
...
function newRuleTable() {
	...
	search_collection(GET['project_id'], ['subject_select', 'object_select'], ['subject', 'object']);
...
function search_collection(project_id, select_holder, type) {
		...
		var url2call = GET['url']+"/S3QL.php?key="+GET['key']+"&query="+q+"&format=json";
		  ...
		  $.getJSON(url2call+'&callback=?', function (collections) {
			...
			for (var s=0; s<select_holder.length; s++) {
				...
				appendToSelectC(collections, select_holder[s], type[s]);
			}
...
function appendToSelectC(data, select_holder, type, selected) {
	for (var i=0; i<data.length; i++) {
				...
				var opt = $(document.createElement('option')).attr('value', data[i].collection_id).html(data[i].name+' (C'+data[i].collection_id+')').appendTo($('#'+select_holder));
	...
	if(typeof(selected)!=='undefined'){
		$("#"+select_holder+" option[selected]").removeAttr("selected");
		if($("#"+select_holder+" option[value='"+selected+"']").length>0){
			$("#"+select_holder+" option[value='"+selected+"']").attr("selected", "selected");
		}
		else {
			$('#'+select_holder).parent().html('<input type="text" id="'+$('#subject_7679').attr('id')+'" value="'+selected+'">');
		}
	}
	
}

function appendToSelectP(data, select_holder, selected) {
	$('#'+select_holder).html('');
...
function objectSelected()
{
	...
	if (selected=='new') {
		...
		search_collection('', 'object_select','object');//search without project
...
function appendNewRule(info) {
		...
		var coldata = {
			...
			'action': '<a href="javascript:s3db_delete('+info.rule_id+', \'R\')">Edit</a>&nbsp;&nbsp;&nbsp;<a href="javascript:s3db_delete('+info.rule_id+', \'R\')">Delete</a>'
...
function s3db_edit(id, E) {
	var colBox = 'subject_'+id;
	var verbBox = 'verb_'+id;
	var objBox = 'object_'+id;

	//what is the current value?
	var c = $('#'+colBox).attr('value');
	var v = $('#'+verbBox).attr('value');
	var o = $('#'+objBox).attr('value');
	
	
	//to each, append a select
	$('#'+colBox).html('').append('<select id="edit_'+colBox+'">');
	$('#'+verbBox).html('').append('<select id="edit_'+verbBox+'">');
	$('#'+objBox).html('').append('<select id="edit_'+objBox+'">');
	
	appendToSelectC(s3dbCollections, "edit_"+colBox, 'subject', c);
	appendToSelectC(s3dbCollections, "edit_"+objBox, 'object', o);
	appendToSelectP(s3dbVerbs, "edit_"+verbBox, v);

	$('#td_action_'+id).html('<input type="submit" onclick="readEditInputs('+id+')" value="Submit">').append('<INPUT TYPE="submit" value="Cancel" onClick="restoreRuleLine('+id+')">');
}

function readEditInputs(id) {
	 var colBox = 'subject_'+id;
	 var subjs= $('#'+colBox).children().children();
		for (var i=0; i<subjs.length; i++) {
			if(subjs[i].selected){
				console.log('coll is '+subjs[i].value)
			}
		}
	var verbBox = 'verb_'+id;
	 var preds= $('#'+verbBox).children().children();
		for (var i=0; i<preds.length; i++) {
			if(preds[i].selected){
				console.log('pred is '+preds[i].value)
			}
		}
	
	var objBox = 'object_'+id;
	 var objs= $('#'+objBox).children().children();
		for (var i=0; i<objs.length; i++) {
			if(objs[i].selected){
				console.log('obj is '+objs[i].value)
			}
		}

		alert('Edit is not working...yet');
}

function restoreRuleLine(id) {
	var info = s3dbRules[id];
	var tr = $('#table_rules_'+info.rule_id);
	var col_heads = ['rule_id', 'subject', 'predicate','object', 'validation', 'notes','created_on', 'created_by','action'];
	tr.html('');
	var coldata = {'rule_id':info.rule_id, 
			'subject':
			'<span id="subject_'+info.rule_id+'" value="'+info.subject_id+'">'+info.subject+' <FONT SIZE="1px">(C'+info.subject_id+')</FONT></span>', 
			'predicate':'<span id="verb_'+info.rule_id+'" value="'+info.verb_id+'">'+info.verb+'<font size="1px">(I'+info.verb_id+')</FONT></span> ', 
			'object':'<span id="object_'+info.rule_id+'" value="'+((info.object_id!=='')?info.object_id:info.object)+'">'+info.object+((info.object_id!='')?'<FONT SIZE="1px">(C'+info.object_id+')</FONT></span> ':''), 'created_on':info.created_on.substring(0, 10), 
		'validation':info.validation,
		'notes':info.notes,
		'action':((info.change)?'<a href="javascript:s3db_edit('+info.rule_id+', \'R\')">Edit</a>&nbsp;&nbsp;&nbsp;<a href="javascript:s3db_delete('+info.rule_id+', \'R\')">Delete</a>':'')
	...
	for (var j=0; j<col_heads.length; j++) {
			...
			if (col_heads[j]==='created_by') {
				//if the data is not ready, don't put anything in html
				$(document.createElement('td')).attr('id','td_'+col_heads[j]+'_'+info.rule_id).appendTo(tr);
				
				$('#td_'+col_heads[j]+'_'+info.rule_id).html(user(info.created_by, 'td_'+col_heads[j]+'_'+info.rule_id));
				
			}
			else {
				var id = 'td_'+col_heads[j]+'_'+info.rule_id;
				$(document.createElement('td')).attr('id',id).appendTo(tr);
				if(coldata[col_heads[j]]) {
					$('#'+id).html(coldata[col_heads[j]]);
				}
			}
